...
// Verbatim content of particle-drift.html — escaped template literal fed to a srcdoc
// iframe. Backticks inside the markup are escaped (\` and \${) so the outer literal
// still parses — a plain unescaped paste breaks the whole file (TS1005).
...
<body class="bg-[#030509] min-h-screen flex items-center justify-center p-4 md:p-12 font-sans antialiased text-[#FFFFFF] overflow-x-hidden selection:bg-[#60A5FA] selection:text-[#030509]">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            // --- ASCII Particle System Implementation (self-contained: runs even if the CDN
            // scripts above are blocked by an offline preview / ad-blocker / CSP) ---
            ...
            // --- GSAP Timeline Reveals (guarded: header reveals, never blocks the canvas) ---
...
export default function ParticleDrift({
  ...
  // Rebuild when baked geometry/mode knobs change. Speed/opacity stay live via postMessage.
